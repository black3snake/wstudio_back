...
class CommentController {
    ...
    static async getComments(req, res) {
        ...
        if (!mongoose.Types.ObjectId.isValid(article)) {
            return res.status(400)
                .json({error: true, message: "Неверный формат ID статьи"});
        }

        const offset = req.query['offset'] || 0;
        // const loadCount = 10;
        const limit = req.query['limit'] || 10;
        const totalCount = await CommentModel.countDocuments({article: article});
    ...
    static async getCommentActions(req, res) {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400)
                .json({error: true, message: "Неверный формат ID комментария"});
        }
